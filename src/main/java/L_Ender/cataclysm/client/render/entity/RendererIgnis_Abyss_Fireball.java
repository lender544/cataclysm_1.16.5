...
public class RendererIgnis_Abyss_Fireball extends EntityRenderer<Ignis_Abyss_Fireball_Entity> {
    ...
    public RendererIgnis_Abyss_Fireball(EntityRendererManager renderManager) {
        super(renderManager);
        this.model = new ModelIgnis_Fireball();
    }

    @Override
    public ResourceLocation getEntityTexture(Ignis_Abyss_Fireball_Entity entity) {
        return IGNIS_FIRE_BALL;
    }

    @Override
    protected int getBlockLight(Ignis_Abyss_Fireball_Entity entityIn, BlockPos pos) {
        return 15;
    }

    @Override
    public void render(Ignis_Abyss_Fireball_Entity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
        matrixStackIn.push();
